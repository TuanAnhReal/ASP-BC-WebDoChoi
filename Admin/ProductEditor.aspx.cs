    ...
    public partial class ProductEditor : System.Web.UI.Page
    {
        ...
        private void LoadDropDownData()
        {
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                // --- LOAD NHÓM SẢN PHẨM (MỚI) ---
                SqlDataAdapter daNhom = new SqlDataAdapter("SELECT MaNhom, TenNhom FROM NhomSanPham", conn);
                DataTable dtNhom = new DataTable();
                daNhom.Fill(dtNhom);
                ddlNhomSP.DataSource = dtNhom;
                ddlNhomSP.DataTextField = "TenNhom";
                ddlNhomSP.DataValueField = "MaNhom";
                ddlNhomSP.DataBind();
                ddlNhomSP.Items.Insert(0, new ListItem("-- Chọn Nhóm --", "0"));
        ...
        private void LoadData(string id)
        {
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                string sql = @"
                    SELECT TOP 1 s.*, b.GiaBan, b.SoLuongTon, a.DuongDanAnh, s.MaNhom
                       FROM SanPham s
                ...
                if (dr.Read())
                {
                    ...
                    txtMoTaChiTiet.Text = dr["MoTaChiTiet"].ToString();
                    ...
                    // Gán giá trị cho DropDownList Nhóm
                    if (dr["MaNhom"] != DBNull.Value)
                    {
                        ddlNhomSP.SelectedValue = dr["MaNhom"].ToString();
                    }
            ...
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                try
                {
                    ...
                    else
                    {
                        // --- LOGIC THÊM MỚI ---
                        // 1. Insert SP
                        string sqlInsertSP = @"INSERT INTO SanPham (TenSanPham, MaNhom, MaThuongHieu, MaDoTuoi, MoTaChiTiet, TrangThai) 
                                               VALUES (@Ten, @MaNhom, @MaTH, @MaDT, @MoTa, 1); SELECT SCOPE_IDENTITY();";

...
namespace WebDoChoi
{
    public partial class Customer : System.Web.UI.MasterPage
    {
        ...
        //load menu nhóm sản phẩm
        private void LoadMenuNhom()
        {
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                SqlDataAdapter da = new SqlDataAdapter("SELECT MaNhom, TenNhom FROM NhomSanPham ORDER BY TenNhom", conn);
                DataTable dt = new DataTable();
                da.Fill(dt);

                rptMenuNhom.DataSource = dt;
                rptMenuNhom.DataBind();
            }
        }
